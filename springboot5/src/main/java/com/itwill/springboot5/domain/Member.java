...
public class Member extends BaseTimeEntity implements UserDetails {
	...
	@EqualsAndHashCode.Include // username 필드를 equals()와 hashCode()를 재정의할 때 사용하겠다는 의미.
	@NaturalId // unique 제약 조건
	@Basic(optional = false) // not null 제약 조건
	@Column(updatable = false) // update 쿼리의 set 절에서 제외하기 위한 설정.
	...
	@ElementCollection(fetch = FetchType.LAZY) // 연관 테이블(member_roles)을 사용하기 위해서.
...
//		}
		...
		List<SimpleGrantedAuthority> authorities = roles.stream()
				...
				.toList();
		// List<SimpleGrantedAuthority> -> List<? extends GrantedAuthority>도 가능.
		return authorities;
	}
