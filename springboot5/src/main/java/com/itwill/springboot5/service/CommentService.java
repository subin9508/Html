...
public class CommentService {
    ...
    @Transactional
    //-> @Transactional 애너테이션을 썼을 때만, 엔터티를 findById 등의 메서드로 검색한 후, 엔터티가 변경되면 자동으로 update 쿼리가 실행됨.
    //-> @Transactional 애너테이션을 썼을 때만, JpaRepository<T, ID>.save(entity) 메서드를 명시적으로 호출할 필요가 없음.
    public void update(CommentUpdateDto dto) {
    	...
    	// commentRepo.save(entity) 메서드를 명시적으로 호출할 필요 없음. (@Transactional 애너테이션을 썼을 때만)
