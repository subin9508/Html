...
public class PostService {
		...
		public Page<PostListItemDto> read(int pageNo, Sort sort) {
			...
			// Pageable 타입의 객체 생성.
			Pageable pageable = PageRequest.of(pageNo, 5, sort); // 한 페이지에 5개씩 보여줌. (설정하기 나름)
			...
			log.info("page.number = {}", list.getNumber()); // 현재 페이지 번호(화면에 보여줄)
			log.info("page.hasPrevious = {}", list.hasPrevious()); // 이전 페이지가 있는 지 여부
			log.info("page.hasNext = {}", list.hasNext()); // 다음 페이지가 있는 지 여부
			
			// Page<Post> 객체를 Page<PostItemDto> 타입으로 변환.
			Page<PostListItemDto> posts = list.map(PostListItemDto::fromEntity); // (x) -> PostListItemDto.fromEntity(x)
		...
		public Long create(PostCreateDto dto) {
			...
			// 영속성 계층(리파지토리)의 메서드를 호출해서 DB insert 쿼리를 실행.
			Post entity = postRepo.save(dto.toEntity());
			log.info("entity = {}", entity);
			
			return entity.getId(); // DB 테이블에 insert된 레코드의 PK(id)를 리턴. 
