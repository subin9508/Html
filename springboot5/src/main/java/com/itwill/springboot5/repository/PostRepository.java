...
public interface PostRepository extends JpaRepository<Post, Long>, PostQuerydsl {
	// JPA Query Method 작성:
	// 제목에 포함된 문자열 대소문자 구분 없이 검색하기:
	Page<Post> findByTitleContainingIgnoreCase(String keyword, Pageable pageable);
	
	// 내용에 포함된 문자열 대소문자 구분 없이 검색하기:
	Page<Post> findByContentContainingIgnoreCase(String keyword, Pageable pageable);
	// 작성자에 포함된 문자열 대소문자 구분 없이 검색하기:
	...
	// 제목 또는 내용에 포함된 문자열 대소문자 구분 없이 검색하기:
	// findByTitleContainingIgnoreCaseOrContentCotainingIgnoreCase(args)
	// findByTitleContainingOrContentAllIgnoreCase(args)
