...
//        console.log(event.target);
        if(!confirm('정말 삭제할까요?')) { // 사용자가 '아니오'를 클릭했을 때, 이벤트 핸들러를 동작시키지 않음.
            return;
        }
        
        const id = event.target.getAttribute('data-id'); // 클릭한 버튼의 attribute 값을 읽어들임.
        ...
        axios.delete(uri)
             ...
             .catch((error) => console.log(error)); //  실패 요청이 왔을 때
    }
...
//        console.log(event.target);
 ...
 //       console.log(textarea);
          ...
          const data = { id, ctext }; // 업데이트 요청 데이터. {id: id, ctext: ctext} , 프로퍼티 이름이 지역변수명과 같으면 간단하게 쓸 수 있음.
          axios.put(uri, data)
               .then((response) => {
                console.log(response);
               alert(`댓글 #${id} 업데이트 성공!`);
