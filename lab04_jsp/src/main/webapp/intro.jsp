...
2. JSP(Java/Jakarta Server Page) 
    ...
    (2) HTML 형식의 파일에서 필요한 부분만 자바 코드들이 실행될 수 있도록 만든 "server-side" 문법. 
    (3) JSP 동작 원리:
        o. 최초 요청 -> jsp 파일을 java 파일로 변환(톰캣이) -> java 코드를 class로 컴파일(WAS가)
            -> 객체 생성 -> 메서드 호출 -> 응답      
        o. 요청 -> 생성된 객체에서 메서드 호출 - > 응답

3. JSP의 구성 요소(문법)
    (1) 주석(comment): jsp 파일이 java 코드로 변환될 때 무시되는 코드. (%와 -- 사이에 공백 있으면 안된다.)
    ...
    (5) 식, 표현식(expression): <%= ... %> -> 가장 많이 사용되는 문법.
