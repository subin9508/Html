...
<body>
    ...
    <main>
        ...
        <jsp:useBean id="contact2"
            class="com.itwill.lab04.model.Contact"></jsp:useBean>
        <%-- Contact contact2 = new Contact();와 같은 코드 --%>
        <%-- import가 되어있더라도 클래스 이름만 쓰면 안되고, 패키지 이름까지 전부 적어주어야 함. --%>

        <jsp:setProperty property="id" name="contact2" value="2" />
        <%-- contact2.setId(2);와 같은 코드 --%>

        <jsp:setProperty property="name" name="contact2" value="오쌤" />
        <jsp:setProperty property="phone" name="contact2"
            value="010-1234-5678" />
        <jsp:setProperty property="email" name="contact2"
            value="jake@itwill.com" />
    ...
    <%-- <%@ include file=""%>와 비슷하지만, 각각의 JSP 파일들이 각각 컴파일된 후 합쳐짐. 
        (따라서, jspf가 아닌 jsp를 사용함.)
     --%>
</body>
</html>