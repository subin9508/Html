...
document.addEventListener('DOMContentLoaded', () => { // document object의 컨텐트들이 모두 로드 됐을 때 실행할 함수.
    //  btnToggleComment 버튼 요소를 찾음.
    const btnToggleComment = document.querySelector('button#btnToggleComment');
//    $('button#btn') : A-jax로 작성 시 문법.

    // collapseComments div 요소를 부트스트랩의 Collapse 객체로 생성. 
    const bsCollapse = new bootstrap.Collapse('div#collapseComments', {toggle: false}); // 생성자에게 문자열과 자바스크립트 객체를 넘김.
    ...
    function registerComment() { // function으로 함수 만들 때, 선언 순서는 중요하지 않다. 함수의 선언과 사용만 정확히 구분.
      ...
      /*  const data = {
       ...
       const data = {postId, ctext, username}; // 프로퍼티 이름과 dto의 필드 이름은 같아야 한다. 
       console.log(data);
