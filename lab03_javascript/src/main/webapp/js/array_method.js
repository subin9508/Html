 ...
 */
 ...
 // slice(start, end): 배열에서 start 인덱스부터 end 인덱스까지를 잘라낸 원소들을 갖는 새로운 배열을 리턴.  
 // -> 원본 배열의 내용이 바뀌지 않음.
 console.log(arr);
 console.log(result);
 ...
 // 배열 numbers의 원소들 중에서 홀수들로만 이루어진 새로운 배열 생성.
 const odds = []; // let odds = [];
...
// 배열 numbers의 원소들의 제곱을 원소로 갖는 새로운 배열 생성.
const squares = [];
for (let x of numbers) {
    squares.push(x * x);
}
...
numbers.forEach((x) => console.log(x)); // for 반복문을 간단하게 쓰는 함수.
