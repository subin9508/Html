 ...
 * 
 * 자바스크립트 함수 선언(정의) 방법:
 * function 함수이름([파라미터, ...]) { // 파라미터 생략 가능.
 *      실행코드;
 *      [return 값;] // return 값 생략 가능.
 * }
 * 
 * 함수 이름 앞에 리턴 타입을 선언하지 않음.
 * 파라미터를 선언할 때는 const, let, var(사용을 권장하지 않음.)와 같은 키워드를 사용하지 않음.
 */
...
// JS의 함수는 파라미터의 타입을 검사하지 않음.
result = add('Hello', 'JavaScript');
console.log(`result = ${result}`);

// JS의 함수는 파라미터의 개수를 검사하지 않음.
...
// JS의 모든 함수는 arguments 속성(attribute)을 가지고 있음.
function testArgs() {
...
// 숫자 2개를 아규먼트로 전달 받아서 뺄셈 결과를 리턴하는 함수.
function minus(x, y) {
    return x - y;
